...
public class GcpLbErrorCountTest {
    ...
    @Test
    @DisplayName("HTTP 및 HTTPS 포워딩 룰이 동일 URL Map에 연결된 경우 HTTPS 대표값(611,345) 선별 및 2배 중복 방지 검증")
    public void testLb500ErrorDeduplication() {
        ...
        for (Map.Entry<String, Map<String, Long>> entry : urlMapToRuleCount.entrySet()) {
            ...
            if (ruleCounts.size() == 1) {
                total500Count += ruleCounts.values().iterator().next();
            } else {
        ...
        assertEquals(611345L, total500Count, "HTTP/HTTPS 중복 합산이 제거되고 실제 HTTPS 에러 건수인 611,345건이 산출되어야 합니다.");
    }

    @Test
    @DisplayName("단일 포워딩 룰 및 다중 독립 URL Map 로드밸런서의 에러 합산 검증")
    public void testMultipleIndependentLbCounts() {
        ...
        assertEquals(1250L, total500Count);
    }
}
